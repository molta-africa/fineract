...
public class MoltaLoanRejectedNotificationListener implements BusinessEventListener<LoanRejectedBusinessEvent> {
    ...
    public void onBusinessEvent(LoanRejectedBusinessEvent event) {
        try {
            ...
            if (client != null && client.mobileNo() != null && !client.mobileNo().isEmpty()) {
                // Check if SMS is enabled for loan rejection
                MoltaSmsLoanConfig moltaSmsLoanConfig = moltaSmsLoanConfigLoader.getMoltaSmsLoanConfig();
                if (!moltaSmsLoanConfig.isRejectEnabled()) {
                    log.info("SMS is not configured to be sent for loan rejection");
                    return;
                }
                ...
                smsText = smsText.replace("${display_name}", client.getDisplayName())
                        ...
                        .replace("${principal_amount}", loan.getPrincipal().toString());
            ...
            log.error("Error queuing SMS notification for loan rejection", e);
        }
    }
}