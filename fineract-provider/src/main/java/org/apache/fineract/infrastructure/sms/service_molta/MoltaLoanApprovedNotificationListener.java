...
public class MoltaLoanApprovedNotificationListener implements BusinessEventListener<LoanApprovedBusinessEvent> {
    ...
    public void onBusinessEvent(LoanApprovedBusinessEvent event) {
        try {
            ...
            if (client != null && client.mobileNo() != null && !client.mobileNo().isEmpty()) {
                // Check if SMS is enabled for loan approval
                MoltaSmsLoanConfig moltaSmsLoanConfig = moltaSmsLoanConfigLoader.getMoltaSmsLoanConfig();
                if (!moltaSmsLoanConfig.isApproveEnabled()) {
                    log.info("SMS is not configured to be sent for loan approval");
                    return;
                }

                // Find the campaign
                String campaignName = moltaSmsLoanConfig.getApprovedCampaign();
            ...
            log.error("Error queuing SMS notification for loan approval", e);
        }
    }
}